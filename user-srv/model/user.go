...
type User struct {
	...
	Status   uint32     `gorm:"default:0;comment:'用户状态,0表示正常用户,1表示禁用用户'"`
	UserType uint32     `gorm:"default:0;comment:'用户类型,0表示普通用户,1表示后台系统管理用户'"`
	Gender   uint32     `gorm:"default:0;comment:'用户性别,0表示男,1表示女'"`
	Birthday *time.Time `gorm:"comment:'用户生日时间'"`
}
